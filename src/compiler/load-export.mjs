...
export async function loadSiteExport(exportDir) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') {
      throw new Error(`sitemap.json not found in ${root}`);
    }
    throw error;
  }

  if (!Array.isArray(sitemap)) {
    throw new Error('sitemap.json must contain an array');
  }
  ...
  return {
    root,
    domain: domainFromPages(sitemap),
    sitemap,
    pages,
    designSystem,
  };
