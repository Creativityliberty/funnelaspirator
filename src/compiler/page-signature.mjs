...
export function buildPageSignature(page = {}) {
  const sectionSequence = (Array.isArray(page.components) ? page.components : []).map(normalizeRole);
  const ctaCount = Array.isArray(page.microInteractions?.ctas)
    ? page.microInteractions.ctas.length
    : 0;
  const formCount = sectionSequence.filter((item) => item === 'form').length;
